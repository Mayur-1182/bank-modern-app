  ...
  return (
    <section className={` ${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
          {footerLinks.map((footerLink) => (
            ...
            >
              <h4
                className={`font-poppins font-medium text-white text-[18px leading-[27px]]`}
              >
                {footerLink.title}
              </h4>
              <ul className="mt-4">
                {footerLink.links.map((link) => (
                  <li
                    key={link.name}
                    className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite cursor-pointer hover:text-secondary my-2`}
                  >
                    {link.name}
                  </li>
                ))}
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px]  ">
        <p className={`${styles.paragraph} text-center font-poppins`}>
